...
const significantFiles = ['artisan', 'svelte.config.js', 'tsconfig.json', 'mix.exs', 'cargo.toml', 'package.json'];
function LocalItem(props: { entry: EntryLike; uri: string; pinned?: boolean } & PinMethods) {
  ...
  const filesProjectContains = new Set<string>();
  const files = readdirSync(path);
  for (let file of files) {
    if (significantFiles.includes(file)) {
      filesProjectContains.add(file);
    }
  }

  let projectType;
  if (filesProjectContains.has('artisan')) {
    projectType = 'laravel';
  } else if (filesProjectContains.has('svelte.config.js')) {
    projectType = 'svelte';
  } else if (filesProjectContains.has('tsconfig.json')) {
    projectType = 'typescript';
  } else if (filesProjectContains.has('mix.exs')) {
    projectType = 'phoenix';
  } else if (filesProjectContains.has('cargo.toml')) {
    projectType = 'rust';
  } else if (filesProjectContains.has('package.json')) {
    projectType = 'javascript1';
  }
  ...
  return (
    <ListOrGridItem
      ...
      title={name}
      subtitle={subtitle}
      icon={typeof projectType === 'string' ? `${projectType}.png` : Icon.Folder}
